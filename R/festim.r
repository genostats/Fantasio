...
festim_fmatrix <- function(x, verbose = TRUE, debug = FALSE) {
  ...
  for(i in seq_len(nrow(x))) {
    ...
    likelihood0 <- .Call('_Fantasio_logLikelihood', PACKAGE = "Fantasio", logEmission, x@delta.dist, 0.01, 0)

    if(verbose) cat("Estimation f and a for individual #",i,"\n")

    last_theta <- c(0.05, 0.05)
    last_likelihood <- .Call('_Fantasio_logLikelihood_gradient', PACKAGE = "Fantasio", logEmission, x@delta.dist, last_theta[1], last_theta[2])
    ...
    f <- function(theta) { 
      ...
      last_theta <<- theta
      last_likelihood <<- .Call('_Fantasio_logLikelihood_gradient', PACKAGE = "Fantasio", logEmission, x@delta.dist, theta[1], theta[2])
