...
makeSubmapsByHotspots <- function(bedmatrix, n = 100, segmentsList = createSegmentsListByHotspots(bedmatrix), n.cores = 1, epsilon = 1e-3, run.festim=TRUE, list.id, run.proba=TRUE,
  ...
  submaps <- setSummary(submaps, run_a_f = run.festim, probs = run.proba, by_segments=recap.by.segments, list.id=list.id, threshold=threshold, q=q, quality=quality, n.consecutive.marker=n.consecutive.marker, test=test)
