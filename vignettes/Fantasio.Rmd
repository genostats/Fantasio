...
Fantasio is a user friendly pipeline, written in R, composed of several functions. Its goals are for:

* Population genetic studies (estimating and detecting inbreeding on individuals without known genealogy, estimating the population proportion of mating types and the
...
In this vignette, we illustrate Fantasio using the data set HGDP-CEPH.  Not  all options of the functions are described here, but rather their basic usage.  The reader is advised to look at the manual pages for details.
...
and searching for a region of the genome of shared homozygosity. Original homozygosity mapping requires that the genealogy of patients be known so that inbred patients can be identified and their respective f estimated. Leutenegger et al. (Leutenegger et al. 2006) proposed using the f estimated on genome-wide genetic data to compute a FLOD score, similar to Morton’s LOD score (Morton 1955). This FLOD score can be computed on isolated cases or on nuclear families.
...
Génin et al. (Genin et al. 2012) proposed to detect fully penetrant rare recessive variants by performing homozygosity mapping on inbred cases from Genome-Wide Association Study (GWAS) data. Linkage evidence is then evaluated over the entire set I of inbred cases by computing a FLOD score, HFLOD(m,α), at each marker m, with a heterogeneity parameter α, that takes into account the possibility that only a fraction of the inbred affected individuals carry diseases causing mutations:
...
Please refer to the vignette of those package for help and especially the one of gaston package.
...
```{r}
suppressMessages(require(Fantasio)) #for the sake of clarity we supress the messages outputed
require(HGDP.CEPH)
```

### Input data file
From now on, we can use the package. Let us first create our bed.matrix object using the data file we load with the package HGDP.CEPH. Your file needs to be a ".bed" one :
```{r}
...
Here we only want to work on the Bedouin's data, so we select this population with this command that returns subset of individuals satisfying a condition :
```{r}
...
As Fantasio computes only for people affected by the disease, we change the data from 0 (=unknown) to 2 (=affected) : 
```{r}
...
We implemanted in the package Fantasio two differents methods in order to create the submaps : 
...
```{r}
submaps <- suppressMessages(makeAllSubmapsByHotspots(x.me, 5, s, verbose=FALSE)) #suppressing messages to avoid unnecesary warnings
```
...
We use the same segment list that is used below (s). 
...
```{r}
submaps0 <- suppressMessages(makeAllSubmapsByHotspots(x.me, 5, s, verbose=FALSE, recap.by.segments = TRUE)) #for clarity sake we suppress unnessary massages.
```
...
We will now create segments, which will be use to create the submaps later, further 
explication below, for now use this command :
...
We will now head toward the creation of submaps using the following commands : 
  ...
  ```{r}
submaps1 <- suppressMessages(makeAllSubmapsBySnps(x.me, 5, s1, verbose=FALSE)) #suppressing messages to avoid unnecesary warnings
```
...
We just saw how to use the Fantasio package step by step.

The function Fantasio is a wrapper to make the usage of the package easier. The function calls two differents functions : ‘createSegmentsListBySnps’ and ‘createSegmentsListBySnps’. The first function ‘createSegmentsListBySnps’ is used to create a list of segments though the genome. The second function makeAllSubmapsBySnps or makeAllSubsmapsbyHotspots is used to create submaps.

Yon can see the description of the functions and their utilisations below. 
...
HBD.plot.id(submaps, individual.id = "HGDP00628", family.id = "HGDP00628")
```
