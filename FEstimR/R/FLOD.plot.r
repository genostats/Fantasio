...
{ 
  ...
  for ( i in unique(x@map$chr)) # pour chaque chr on fait un plot 
  {
      ...
      #plotting
      ymax <- max(3, max(x@HBD.prob[, x@map$chr == i], na.rm = TRUE)) 
      xmax <- max(d, na.rm = TRUE)
      ymin <- min(x@HBD.prob[, x@map$chr == i], na.rm = TRUE)
      png(file = paste("Plots/HBD.prob.","cM.",i,".png",sep = ""), width = 4000, height = 2500, pointsize=24)
          
      #d=distance pour chaque chr i ; r=HBD.prob pour chaque Indiv j & chr i
      plot(d, r, type="n", pch=16, xlim = c(0,xmax), ylim = c(ymin, ymax), xlab = "Position (cM)", ylab="HBD.prob", cex.lab=1.4, cex.axis=1.5, main= paste("HBD.prob (chromosome ",i,")", sep=""), cex.main=1.5)    
      ...
      #Obtenir les HBD.prob de chaque individu
      for ( j in 1:x@nrow )
        
      {
        if(x@f[j] == 0) next
        r <- x@HBD.prob[j, x@map$chr == i]
        lines(d, r,col= j)
  ...
  ymax <- max(3,x@HBD.prob, na.rm = TRUE)
  ymin <- min(x@HBD.prob, na.rm=TRUE)
  ...
  {
    if(x@f[i] == 0) next
    png(file = paste("Plots/HBD.prob.Individu",x@ped$famid[i], ".", x@ped$id[i],".cM.genome.png",sep=""), width = 2400, height = 800, pointsize = 24 )
    plot(res, x@HBD.prob[i,], type="b",pch=16, lty=1,xlim=c(0,xmax), ylim = c(ymin,ymax), xlab = "", ylab = "HBD.prob", cex.lab = 1.4, cex.axis=1.5, main= paste("HBD.prob : individu ", x@ped$famid[i], "_", x@ped$id[i], sep=""), col= mycol[x@map$chr], xaxt="n", cex=0.75 )  
